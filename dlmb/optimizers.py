...
# For all classes optimizations[0] is the calculated gradients W.R.T the vars from the layer, and anything other than that is just optimizations from the optimizer class.
...
class Base_optimizer(metaclass=ABCMeta):
	...
	# ------------------------------------------------------------------------------------------------------------------------
	@abstractmethod
	def map_data(self, optimizations, epoch):
		"""
...
class GD(Base_optimizer):
	def __init__(self, lr=0.1, momentum=0.9):
		...
		Arguments:
			lr - float: A float that takes a small fraction of the gradients. Controls how fast the model learns, however choosing a big lr isn't always good.
			momentum - float: A float that controls the speed of convergence to a local minima, however choosing a big momentum isn't aslways good.
	...
	def map_data(self, optimizations, epoch):
		...
		Arguments:
			optimizations - list: A list containing:
				gradients - Numpy array: A numpy array of gradients for the vars that are being updated.
				optimizations - Numpy array: A numpy array of optimizations from the previous time step.

		Returns:
			output - list: A list containing:
				updates - Numpy array: A numpy of the optimized gradients that will be used to update the vars.
				optimizations - Numpy array: A numpy array of optimizations from this time step.
...
class RMSprop(Base_optimizer):
	def __init__(self, lr=0.001, beta=0.99, epsilon=1e-8):
		...
		Arguments:
			lr - float: A learning rate that takes a small fraction of the gradients. Controls how fast the model learns, however choosing a big lr isn't always good.
			beta - float: Helps the gradients find the optimal direction to head in.
			epsilon - float: A very small number that helps to stop division by zero.
...
class Adam(Base_optimizer):
	def __init__(self, lr=0.01, momentum=0.9, beta=0.99, epsilon=1e-8):
		...
		Arguments:
			lr - float: A learning rate that takes a small fraction of the gradients. Controls how fast the model learns, however choosing a big lr isn't always good.
			momentum - float: How much momentum to add to the gradients.
			beta - float: Helps the gradients find the optimal direction to head in.
			epsilon - float - float: A very small number that helps to stop division by zero. 
	...
	def map_data(self, optimizations, epoch):
		...
		Arguments:
			optimizations - list: A lsit containing:
				gradients - Numpy array: A numpy array of gradients for the vars that are being updated.
				beta - Numpy array: A numpy array of beta optimizations from the previous time step.
				momentum - Numpy array: A numpy array of momentum optimizations from the previous time step.

		Returns:
			output - list: A list containing:
				updates - Numpy array: A numpy of the optimized gradients that will be used to update the vars.
				beta - Numpy array: A numpy array of beta optimizations from this time step.
				momentum - Numpy array: A numpy array of momentum optimizations from this time step.
...
def get(optimizer):
	"""
	Finds and returns the correct optimizer class.

	Arguments:
		optimizer - str/instance of a class: The optimization class.

	Returns:
		optimizer - instance of a class: The correct optimization class.
		
	"""

	if type(optimizer) == str:
		if optimizer.lower() in ("gd"):
			return GD()
			...
			print("'%s' is not currently an available optimizer. Has been set to 'GD' by default" % optimizer)
			return GD()
	else:
		return optimizer
